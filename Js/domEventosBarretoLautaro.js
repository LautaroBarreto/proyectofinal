...
const filtro = (array, alteracion) => {return array.filter(item => item.alteracion == alteracion) }


const handleClick = (array, select) => {
    return filtro(array, select.value);
}
...
mostrar.onclick = () => {
    viewHTML(handleClick(todasLasEscalas, selectOptions), viewResult);
}